	...
	for {
		...
		case watchResp := <-workCh:
			...
			for _, event := range watchResp.Events {
				switch event.Type {
				case mvccpb.PUT:
					latestMetaWork.Store(event.Kv.Value)
				case mvccpb.DELETE:
					var nilBytes []byte
					latestMetaWork.Store(nilBytes)
				}
			}
...
func (ma *metaAdmin) etcdJoin(name string) error {
	...
	sub := metaapi.SubscribeToEtcdCluster(ch)
	defer func() {
		sub.Unsubscribe()
		close(ch)
	}()
...
// staggered auto join
func (ma *metaAdmin) etcdAutoJoin() error {
	select {
	...
	if states := getMiners("", 0); len(states) > 0 {
		var tobes []*metaapi.MetadiumMinerStatus
	...
	var state *metaapi.MetadiumMinerStatus
...
again:
	...
	txresp, err := tx.If(
		...
		clientv3.OpPut(metaTokenKey, string(value)),
	).Else(
		clientv3.OpGet(metaTokenKey),
	).Commit()
	...
	if err == nil && !txresp.Succeeded {
		...
		if tokenFound {
			...
			_, err := tx.If(
				...
				clientv3.OpDelete(metaTokenKey),
			).Commit()
			if err != nil {
				return nil, err
			}
...
func (ma *metaAdmin) acquireTokenSync(ctx context.Context, height *big.Int, parentHash common.Hash, ttl int64) (*MetaToken, error) {
	...
	workExists := true
	prevWork := ""
	if prevWorkBytes, err := json.Marshal(&metaWork{Height: height.Int64() - 1, Hash: parentHash}); err != nil {
		return nil, err
	} else {
...
again:
	...
	if workExists {
		txIf = tx.If(
			clientv3.Compare(clientv3.CreateRevision(metaTokenKey), "=", 0),
			clientv3.Compare(clientv3.Value(metaWorkKey), "=", prevWork),
		)
	} else {
		txIf = tx.If(
			clientv3.Compare(clientv3.CreateRevision(metaTokenKey), "=", 0),
			clientv3.Compare(clientv3.CreateRevision(metaWorkKey), "=", 0),
		)
	}
	txresp, err := txIf.Then(
		clientv3.OpPut(metaTokenKey, string(lockValue)),
	).Else(
		clientv3.OpGet(metaTokenKey),
		clientv3.OpGet(metaWorkKey),
	).Commit()
...
func (lck *MetaToken) releaseTokenSync(ctx context.Context, height *big.Int, hash, parentHash common.Hash) error {
	...
	if data, err := json.Marshal(&metaWork{Height: height.Int64() - 1, Hash: parentHash}); err != nil {
		return err
	} else {
		prevWork = string(data)
	}
	if data, err := json.Marshal(&metaWork{Height: height.Int64(), Hash: hash}); err != nil {
		return err
	} else {
		work = string(data)
	}
...
again:
	...
	if exists {
		...
		txIf = tx.If(
			clientv3.Compare(clientv3.Value(lck.Key), "=", lockValue),
			clientv3.Compare(clientv3.Version(metaWorkKey), "=", 0),
		)
	}
	txresp, err := txIf.Then(
		clientv3.OpDelete(lck.Key),
		clientv3.OpPut(metaWorkKey, work),
	).Else(
		clientv3.OpGet(lck.Key),
		clientv3.OpGet(metaWorkKey),
	).Commit()
...
func EtcdGetWork() (string, error) {
	...
	return admin.etcdGet(metaWorkKey)
}
...
func EtcdDeleteWork() error {
	...
	return admin.etcdDelete(metaWorkKey)
}
