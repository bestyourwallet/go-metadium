...
var (
	...
	// the latest work info: []byte, json'ed metaWork
	latestMetaWork atomic.Value
	...
	// sync.Map[string]*metaapi.MetadiumMinerStatus
...
func (ma *metaAdmin) handleNewBlocks() {
...
// updates 'miningPeers' upon receiving MetadiumMinerStatus from peers
func handleMinerStatusUpdate() {
	ch := make(chan *metaapi.MetadiumMinerStatus, 128)
...
func loadMiningToken() *MetaToken {
	var (
		lck *MetaToken
		ok  bool
	)
	if lckData := miningToken.Load(); lckData != nil {
		if lck, ok = lckData.(*MetaToken); !ok {
			return nil
		}
	}
	return lck
...
func findConsensusBlock(states []*metaapi.MetadiumMinerStatus) (height *big.Int, hash common.Hash) {
	type anon struct {
		height *big.Int
		hash   *common.Hash
		count  int
...
//  0. updates of 'latestBlock' of 'latestMetaWork' hasn't occurred over 30 seconds
...
func syncCheck() error {
	...
	// check token string
	if tokenData, err := admin.etcdGet(metaTokenKey); err == nil {
		var token = &MetaToken{}
		if err = json.Unmarshal([]byte(tokenData), token); err != nil {
			// invalid token string
			err = admin.etcdDelete(metaTokenKey)
	...
	if workData, err = admin.etcdGet(metaWorkKey); err != nil {
		return err
	} else {
	...
	if newWorkData, err := json.Marshal(newWork); err != nil {
		...
		admin.etcdPut(metaWorkKey, string(newWorkData))
	}
