...
func (ma *metaAdmin) collectMinerStates(height *big.Int) []*metaapi.MetadiumMinerStatus {
	var (
		ctx context.Context
	...
	var states []*metaapi.MetadiumMinerStatus
	for _, n := range e.nodes {
		if s, ok := miningPeers.Load(n.Name); ok {
			if status, ok := s.(*metaapi.MetadiumMinerStatus); ok {
...
func getCoinbaseEnodeCache(ctx context.Context, height *big.Int, gov *metclient.RemoteContract) (*coinbaseEnodeEntry, error) {
	...
	for i := int64(1); i <= count.Int64(); i++ {
		...
		e.nodes = append(e.nodes, &metaNode{
			Name:  string(name),
			Enode: string(enode), // note that this is not in hex unlike metaAdmin
			Id:    idv4,
			Addr:  addr,
		})
...
func (ma *metaAdmin) verifyMinerLimit(ctx context.Context, height *big.Int, gov *metclient.RemoteContract, coinbase *common.Address, enode []byte) (bool, error) {
	// parent block number
...
func (ma *metaAdmin) isEligibleMiner(height *big.Int) (bool, error) {
	var (
		ctx   context.Context
		enode []byte
		gov   *metclient.RemoteContract
...
// beyond modifiedBlock + 1.
// Not enforced if member count <= 2.
func (ma *metaAdmin) nextMinerCandidates(height *big.Int) ([]*metaNode, error) {
	var (
		ctx context.Context
		gov *metclient.RemoteContract
		err error
...
func (ma *metaAdmin) electNextMiner(height *big.Int) error {
	tstart := time.Now()
	enode, err := hex.DecodeString(ma.self.Enode)
	if err != nil {
	...
	closeEnough := int64(2)
	peersMap := map[string]*metaapi.MetadiumMinerStatus{}
	if len(peers) > 0 {
		for _, p := range peers {
